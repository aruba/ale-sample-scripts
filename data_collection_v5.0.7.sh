       ...
       LogDate=$(echo ${a2[$n]})                             # read n'th LogDate from a2
        ...
        do
            ...
            start=$(egrep -o "count=.*" /tmp/temp.log | awk '{print $1}' | cut -d '=' -f 2 | cut -d ',' -f 1 | sort -n | head -n1)
              if [ -z "$start" ];
                 then echo "NULL START date - no value!"
              fi
            
            end=$(egrep -o "count=.*" /tmp/temp.log | awk '{print $1}' | sort -n | cut -d '=' -f 2 | cut -d ',' -f 1 | sort -n | tail -n1)
			  if [ -z "$end" ];
                 then echo "NULL END date - no value!"
              fi
              
			  #check if VAR $start is NULL and print NULL to output file if TRUE
			  if [ -z "$start" ]; 
  			    then printf "NULL!," >> $FILENAME
  			    else
  			       #if $start NULL is FALSE, perform arithmetic subtraction of $start - $end
      			   if [ "$end" -ge "$start" ];
...
# LogQuery3 - dump of log messages for queried time period 
